...
ggplot(Churn, aes(x = Churn))+
  geom_histogram(stat = "count", fill = c("red", "blue"))
#Customer tenure before they Churn
#Visually we can see that probability of it happening is morein short tenure
Churn %>% filter(Churn == "Yes") %>% 
  ggplot( aes(x=  tenure))+
  geom_bar(fill = "blue" )
...
ggplot(meltedDComp, aes(x= X1, y = value)) + 
...
plot(Dtree)
Dtree1 <- ctree(Churn~., trainingData)
plot(Dtree)
plot(Dtree1)
...
FBeta_Score(testData$Churn, pred_random, positive = "Yes", beta = 1.5)
...
FBeta_Score(testData$Churn, Ensemble_pred1, positive = "Yes", beta = 1.5)
